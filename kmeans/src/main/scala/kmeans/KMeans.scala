...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val x: GenMap[Point, GenSeq[Point]] = points.groupBy(point => findClosest(point, means))
    val y: GenMap[Point, GenSeq[Point]] = (for (mean <- means if !x.contains(mean)) yield (mean, GenSeq[Point]())) toMap
