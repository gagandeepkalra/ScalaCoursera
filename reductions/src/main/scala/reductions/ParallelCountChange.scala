...
object ParallelCountChange {
    ...
    money.signum match {
      ...
      case 1 =>
        if (coins.isEmpty) 0
          ...
          countChange(money, coins.tail) + countChange(money - coins.head, coins) // exclude + include
    }
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins) || money <= 0 || coins.isEmpty) countChange(money, coins)
      ...
      val (exclude, include) = parallel(parCountChange(money, coins.tail, threshold), parCountChange(money - coins.head, coins, threshold))
      exclude + include
    }
  }
