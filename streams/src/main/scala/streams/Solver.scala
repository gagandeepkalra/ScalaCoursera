...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    ...
    if (initial.nonEmpty) {
      val neighbours: Stream[(Block, List[Move])] = newNeighborsOnly((neighborsWithHistory _).tupled(initial.head), explored)

      initial.head #:: from(initial.tail ++ neighbours, explored + initial.head._1)
    } else Stream.empty
  }
